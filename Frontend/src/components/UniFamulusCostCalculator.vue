  ...
  <div class="scc-overlay" @click.self="$emit('close')">
    <div class="scc-modal">
      <div class="scc-header">
        <h5 class="mb-0">Uni‑Famulus kalkulátor — {{ event?.nev || 'Rendezvény' }}</h5>
      ...
      <div class="scc-body">
        ...
        <div v-else>
          ...
          <div class="d-flex justify-content-between align-items-center mb-2">
            ...
            <div class="text-end">
              <div class="h6 mb-0">UF összesen: {{ formatMoney(total) }}</div>
            </div>
          </div>
          ...
          <div v-for="(row, idx) in rows" :key="row.id" class="d-flex gap-2 align-items-center mb-2 row-with-trash">
            ...
            <input type="number" min="0" step="0.25" class="form-control form-control-sm" style="width:110px;"
                   v-model="row.hours" placeholder="óra" title="Órák" />
            <input type="number" min="0" step="1" class="form-control form-control-sm" style="width:90px;"
                   v-model="row.persons" placeholder="fő" title="Fő (létszám)" />
          ...
          <div class="d-flex gap-2 mt-3">
            ...
            <button class="btn btn-primary" :disabled="rows.length===0 || saving" @click="commit">Mentés (UF)</button>
          </div>
...
export default {
  ...
  setup(props, { emit }) {
    const TARGET_STATUS = 'UF_ARAJANLAT_ELFOGADASARA_VAR'; // a meglévő logikát követve
    ...
    const filteredPricesFor = (row) => prices.value.filter(p => isUfCategory(p.category));
    ...
    // UF: implicit mezők – az UF kalkulátorban jellemzően óra és fő
    ...
    const commit = async () => {
      ...
      try {
        ...
        saveState();
        emit('status-updated', { id, statusz: TARGET_STATUS });  // azonnali státusz UI
        emit('refresh-events');                                   // lista/parent frissítés
